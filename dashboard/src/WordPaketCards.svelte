<script>
    ...
    function selectTop3(rows) {
        ...
        const highList = distinct
            .filter(
                (r) =>
                    isHigh(r) &&
                    !(absurdList[0] && r.paket === absurdList[0].paket),
            )
            .sort((a, b) => (b.pagu ?? 0) - (a.pagu ?? 0));
    ...
    function labelOf(rec) {
        ...
        const isi = (rec.isInappropriate || rec.tags?.isInappropriate || "")
            .toString()
            .toLowerCase();
    ...
    <ul class="cards" role="list">
        {#each top as rec, i (i)}
            ...
            <li class="card">
                ...
                <div class="card-kapan">{rec.pemilihanDate || "—"}</div>
                <div class="card-paket">
                    {rec.paket || rec.name || "(tanpa nama)"}
                </div>
                <div class="card-foot">
